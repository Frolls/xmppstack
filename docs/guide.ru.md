...
- опциональный `nginx` для HTTP/HTTPS-входа на `80/443`
...
- `roles/nginx/`
  Опциональный конфиг nginx, Quadlet и lifecycle сервиса.
- `roles/certbot/`
...
- `ejabberd`, `coturn` и опциональный `nginx` сначала раскладывают свои unit-файлы и конфиги
...
- `xmpp_stack_enable_nginx`
  Включает опциональный reverse-proxy слой на `80/443`.
...
сертификаты и затем перезапускает `ejabberd`, `coturn` и `nginx`, если proxy-слой включён.
...
- `5443/tcp` для HTTP upload и WebSocket у ejabberd, публично только если `nginx` отключён
- `3478/tcp,udp` для STUN/TURN
- `5349/tcp` для TURN over TLS
- `49152-49200/udp` для TURN relay
- `80/tcp` для standalone ACME у `certbot`
- `443/tcp` для `nginx`, если он включён
...
`certbot --standalone` занял порт `80`, если `nginx` включён.
